...
func _process(delta):
	...
	if parent.any_action_pressed():
		...
		# peelout (Sonic only)
		elif (parent.movement.x == 0 and parent.inputs[parent.INPUTS.YINPUT] < 0 and (parent.character == Global.CHARACTERS.SONIC or parent.character == Global.CHARACTERS.TAILS)):
			parent.sfx[2].play()
			parent.sfx[2].pitch_scale = 1
